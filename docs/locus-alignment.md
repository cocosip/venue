...
| Venue counterpart | Current working tree, HEAD `1b97bc2`, with the SQLite storage-engine migration applied (one database file per tenant through `pkg/sqlite`); every conclusion here follows the code on disk |
...
| 7 | Watcher | `PRESENT` | `locus/src/Locus.Storage/FileWatcherOptionsManager.cs:51-192`, `locus/src/Locus.Storage/FileWatcherAutoManager.cs:63-192`, `locus/src/Locus.Core/Models/FileWatcherConfiguration.cs:43`, `:110-141` | `pkg/watcher/background_file_watcher_service.go:227-554`, `pkg/watcher/file_watcher_auto_manager.go:135-386`, `pkg/watcher/file_watcher.go:1087-1120`, `:1135-1166`, `:1666-1688`, `:1694-1752`, `venue.go:632-638`, `:655-661`, `:689-695`, `config/config.go:238-268`, `:309-337` | Global option persistence, root-configuration derivation, `UpdateWatcher`, `GetWatchersForTenant`, the advanced knobs and the construction-time `InitialEnabled` disable are all aligned |
...
| U3 | Whether the Locus `FileWatcher.cs` (`locus/src/Locus.Storage/FileWatcher.cs`, 85 KB) import pipeline still contains behavior that Venue does not cover | This revision verified only the Venue-side `discoverFiles`/`importFile`/`reserveImportSlot` structure and the Locus option/manager contracts, without comparing the 1764-line scan loop section by section |
...
Two findings of the documentation review remain open; both are also recorded as code-adjacent debt in `docs/architecture.md`.

| Location | Description | Evidence |
| --- | --- | --- |
| `README.md:282` | The defaults table records a watcher minimum file age of `5s`, which the configuration path does apply, but a direct `RegisterWatcher` call with `MinFileAge=0` falls back to **3s** | `README.md:282` vs `pkg/watcher/file_watcher.go:405-406` (3s fallback) and `config/config.go:839` (5s applied by `ApplyDefaults` for a watcher, `:810` for a root template); see `docs/architecture.md` §10.2 |
