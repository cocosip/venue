...
| `pkg/watcher` | Watched-directory imports: scanning, deduplication, import, post-import actions, history persistence (with optional prune throttle and flush debounce), global option persistence, root derivation, and the advanced stability/cache knobs | `pkg/watcher/file_watcher.go`, `background_file_watcher_service.go`, `file_watcher_auto_manager.go`, `watcher_state.go` | `watcher.NewFileWatcher` (`pkg/watcher/file_watcher.go:184`), `NewBackgroundFileWatcherService` (`pkg/watcher/background_file_watcher_service.go:227`), `NewFileWatcherAutoManager` (`pkg/watcher/file_watcher_auto_manager.go:91`) | It does not own watcher definitions (`README.md:598-599`) and does not select physical paths (it goes through `StoragePool.WriteFile`, `pkg/watcher/file_watcher.go:953`) |
...
- Service list (dependency order): health check → cleanup → orphan recovery → file watcher → metadata backup → statistics output (`venue.go:953-964`).
...
- Only when `running`, stop in reverse order: statistics output → metadata backup → file watcher → orphan recovery → cleanup → health check (`venue.go:1004-1023`).
...
| **fingerprint** | `size:mtime.UnixNano()` (`:1255-1260`); `-` when it cannot be computed (`:36-38`) |
| **In-flight deduplication** | In-memory `sync.Map` + `InFlightToken` (`:82-100`, `:137-148`); `reserveImportSlot` uses `LoadOrStore`/`CompareAndSwap` with a unique token (token generation `:942`, sequence `:1548-1553`, reservation `:1482-1523`); `releaseImportSlot` uses `CompareAndDelete` only when the token matches (`:1525-1546`) |
| Import | `storagePool.WriteFile` (`:953`); the source handle is closed before post-import actions run (Windows semantics, `:958-962`) |
| **Post-import actions** | `performPostImportAction` `:987-1016`: `Delete` → `os.Remove` (`:993-994`); `Move` → create the directory + `moveWithoutOverwrite` (`:996-1006`, collision suffix `:1018-1043`); `Keep` → leave in place (`:1008-1010`). A failed action means the history is **not** recorded (ordering invariant `:896-898`, `:958-969`) |
| History policy | After a successful `Delete`/`Move` the history entry is removed (the source path no longer exists, `:971-975`); `Keep` writes the history (`:976-978`) |
...
| Background scan service | Option precedence: persisted document > `ServiceOptions` > deprecated direct fields > defaults (`pkg/watcher/background_file_watcher_service.go:210-226`, resolution `:255-306`); construction-time disable through `InitialEnabled` (`:115-126`, applied `:276-279`); scheduling by watcher due time (`:630-714`, `:828-881`); parallel-scan cap (`:480-489`, `:715-767`); polling-interval clamping (`:901-921`); when disabled it re-checks every `DisabledCheckInterval` instead of exiting (`:605-614`) |
...
Constructors and fluent methods: `config/fluent.go:148` (FileWatcherService), `:185` (FileWatcherRoot), `:358` (Statistics), `:413` (StatisticsDimension), `:442` (StatisticsOutput), `:475` (RetryPolicy), `:505` (TenantManager), `:523` (Metadata), `:541` (Sqlite), `:669` (Volume), `:696` (Tenant), `:719` (FileWatcher), `:853` (Cleanup), `:986` (DeadLetter), `:1016` (RetiredVolumes). Top level: `New`/`DefaultConfig` (`config/config.go:526`, `:531`); the SQLite engine configuration has its own chainable set (`config/fluent.go:64`, `:541`, `:547`, `:553`, `:559`, `:565`, `:572`, `:578`, `:584`, `:590`, `:597`, `:603`, `:610`, `:619`, `:626`, `:633`).
...
The schema is idempotent and applied on every first access: one `files` table with `file_key` as the primary key plus four status/ordering indexes and one partial lease index, and a `schema_meta` row recording the logical revision (`pkg/metadata/sqlite_repository.go:66-134`; a database written by a newer schema revision is refused, `:1623-1650`).
...
The README documents the reclaim bounds (`README.md:545-555`), startup volume preparation (`README.md:844-855`) and the Locus divergences, including backup-based recoverability and the fail-fast switch (`README.md:895-937`), as implemented, so no stale "not implemented" bullet remains in those areas. Three mismatches remain.

| # | Location | Problem | Evidence |
| --- | --- | --- | --- |
| U1 | `README.md:282` | The defaults table states "Watcher minimum file age `5s`", which is what the configuration path yields, but a direct `RegisterWatcher` call with `MinFileAge=0` falls back to **3s** | `README.md:282` vs `pkg/watcher/file_watcher.go:405-406` (3s fallback in `prepareConfiguration`) and `config/config.go:839` (5s applied by `ApplyDefaults` for a watcher; `:810` for a root template) |
